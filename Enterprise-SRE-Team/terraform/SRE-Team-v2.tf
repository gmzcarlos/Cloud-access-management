...
# Create Cloudant administrator policy template
resource "ibm_iam_policy_template" "Cloudant_Administrator_ap_template" {
...
resource "ibm_iam_trusted_profile_template" "SRE_Team_tp_template_v2" {
  template_id = split("/", ibm_iam_trusted_profile_template.SRE_Team_tp_template.id)[0]
  name = ibm_iam_trusted_profile_template.SRE_Team_tp_template.name
  description = ibm_iam_trusted_profile_template.SRE_Team_tp_template.description
  profile {
    name = "Enterprise SRE Team"
    rules {
      type = "<rule type>"
  ...
  policy_template_references {
	id = split("/", ibm_iam_policy_template.MongoDb_Administrator_ap_template.id)[0]
	version = ibm_iam_policy_template.Cloudant_Administrator_ap_template.version
  }
  committed = true
}
...
resource "ibm_iam_trusted_profile_template_assignment" "tp_assignment_instance" {
  template_id = split("/", ibm_iam_trusted_profile_template.SRE_Team_tp_template.id)[0]
  template_version = ibm_iam_trusted_profile_template.SRE_Team_tp_template_v2.version
  target_type = "AccountGroup" // or "Account"
  target = "<account group id>" // or "<account id>"
  depends_on = [
    ibm_iam_trusted_profile_template.SRE_Team_tp_template_v2
  ]
}
